...
The project follows Data Lakehouse best practices, using Delta Lake, Unity Catalog and the **Bronze → Silver → Gold** architecture.
...
├── 📁 .databricks
│   └── 📁 pipeline
│       ├── 📁 html # contains Databricks files in .html format
│       └── 📁 notebooks # contains Databricks files in .ipynb format
├── 📁 datasets 
│   ├── 📁 ai_dataset # contains the dataset generated by the OpenAI 5.0 model
│   └── 📁 olist_dataset # contains the Brazilian E-Commerce Public Dataset by Olist
├── 📁 dbdiagram # contains the code created in dbdiagram.io
├── 📁 images
│   ├── 📁 databricks # Databricks evidence
│   ├── 📁 dbdiagram # dbdiagram.io schema
│   └── 📁 logo # 100cep Gateway logo
```
...
Our goal is to enable **fast**, **secure** and **borderless** transactions — after all, we are _100cep_: with no _city_, _state_ or _country_ limiting the flow of payments.

### 🔑 Key Features
- 🌍 **Global Payments**: Processing without geographic restrictions
- ⚡ **High Performance**: Infrastructure prepared for high transaction volume
- 🔒 **Security**: Real-time fraud and chargeback monitoring
- 📊 **Analytics**: Dashboards and metrics for decision-making
...
⚠ No web scraping or sensitive data was used.  
⚠ No internal or confidential data from real companies was used.

📸 **Evidence**: Screenshots of the collection process are available in the `/docs/images/databricks/` folder.
...
📄 **Complete documentation**: [/docs/data_catalog.md](docs/data_catalog.md)
...
📄 **Complete ETL documentation**: [/docs/etl.md](docs/etl.md)  
...
📸 **Evidence**: Screenshots available in `/docs/images/databricks/`
